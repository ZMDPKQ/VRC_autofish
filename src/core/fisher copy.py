...
class Fisher:
    def __init__(self, roi=None, overlay=None):
        ...
        self.forg_countdown = None  # 收青蛙倒计时
        self.sell_countdown = None  # 卖鱼倒计时
                ...
                if not (targetbar and fish and totalbar):
                    ...
                    if temp_empty_count == 6:
                        if self.fish_finish_wait_time == -1:
                            self.fish_finish_wait_time = state_machine_start_time
                        if state_machine_start_time - self.fish_finish_wait_time >= self.max_fish_finish_wait_time:
                            self.fish_is_success = (self.fish_progress >= 0.8)
                            self.change_state(FishingState.FINISH,state_machine_start_time)
                            self.fish_finish_wait_time = -1
                        ...
                        '''
                        if totalbar:
                            ttb_dx = (total_box[2] + total_box[0])/2 - screen_width/2
                            ttb_dy = (total_box[3] + total_box[1])/1 -screen_height/2
                            if abs(ttb_dx) <= ttb_dx/(screen_width/2)*0.2:
                                ttb_dx = 0
                            if abs(ttb_dy) <= ttb_dy/(screen_height/2)*0.2:
                                ttb_dy = 0
                    ...
                    h_pix_tr = screen_height/1440
                    w_pix_tr = screen_width/2560
                    fx1 = fx1 * w_pix_tr
                    fy1 = fy1 * h_pix_tr
                    fx2 = fx2 * w_pix_tr
                    fy2 = fy2 * h_pix_tr
                    ...
                    if self.bar_prev_cy is not None and self.bar_prev_time is not None:
                        ...
                        if dt_bar > 0.001:
                            ...
                            self.bar_velocity = 0.8 * raw_vel + 0.2 * self.bar_velocity
                    else:
